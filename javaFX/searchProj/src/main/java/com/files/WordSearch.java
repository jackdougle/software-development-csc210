 ...
 */
 ...
 public class WordSearch {
     private static WordList myList = null;
     private static WordGrid myGrid = null;
     ...
     public static void addWord(String[][] letterGrid, WordList myList, String word) {
         ...
         if (works(letterGrid, word, yStart, xStart, yIncr, xIncr)) applyWord(letterGrid, myList, word, yStart, xStart, yIncr, xIncr);
         else addWord(letterGrid, myList, word);
     }
     ...
     public static void applyWord(String[][] letterGrid, WordList myList, String word, int yStart, int xStart, int yIncr, int xIncr) {
         ...
         myList.addWord(word.toUpperCase());
     ...
     public static void readInputs(WordGrid myGrid, WordList myList) {
         ...
         while (!myList.isEmpty()) {  // Go until all words have been found
             ...
             if (searchWord(letterGrid, dir, word, y, x)) removeWord(letterGrid, myList, dir, word, y, x);
     ...
     public static boolean searchWord(String[][] letterGrid, String dir, String word, String col, String row) {
         ...
         yStart = Integer.valueOf(row); xStart = (int) col.strip().charAt(0) - 97; 

         for (int i = 0; i < word.strip().length(); i++) {
     ...
     public static void removeWord(String[][] letterGrid, WordList myList, String dir, String word, String col, String row) {
         // Removes a word from the word grid and word myList and replaces it with asterisks
         ...
         myList.removeWord(word.toUpperCase());
     ...
     public static void main(String[] args) throws IOException {
         ...
         myGrid = new WordGrid(lines.remove(0));
         myGrid.makeGrid();
         String[][] letterGrid = myGrid.getGrid();
 
         while (lines.size() > 0) addWord(letterGrid, myList, lines.remove(0));
         
         myGrid.fillGrid();
         
         readInputs(myGrid, myList);
     }
